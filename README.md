...
该入口先识别并删除频谱形态异常整包，再将保留包按采集顺序每20包重组一窗。正式算法依次执行逐包幅度和归一化、窗口内逐子载波均值、每包减窗口均值、三层单边ACF和非零延迟运动统计量。`phi_stream_mean`是两条Rx流的平均尺度曲线，`Phi_stream_sum`是论文规定的跨流求和结果；每条Rx的`stream_1_phi`和`stream_2_phi`也单独保存。异常识别本身使用临时归一化频谱来避免把共同AGC增益当成形态异常，这个诊断副本不会作为论文算法输入。
