...
该入口先识别并删除频谱形态异常整包，再将保留包按采集顺序每20包重组一窗。正式算法依次执行逐包幅度和归一化、窗口内逐子载波均值、每包减窗口均值、多层ACF和非零延迟运动统计量。图中`Phi_text_average`按照论文正文的“窗口平均”处理时间，并按照论文规定跨Rx流直接求和，不除以2；`Phi_formula_sum`严格按照式(19)在20包上求和后再跨流求和。`Phi_literal_formula_sum`还严格执行式(17)的印刷字面，而递归ACF路径依据正文和Fig. 5。每条Rx的`stream_1_phi`和`stream_2_phi`也单独保存。异常识别本身使用临时归一化频谱来避免把共同AGC增益当成形态异常，这个诊断副本不会作为论文算法输入。
